Feature: The alter table stored procedure adds, removes columns and partitions as necessary preserving the table data.
  ...
  Scenario Outline: Table has required number of partitions
    Given the <table_name> table is created with <initial_partition_count> partitions and <row_count> rows
    When the <table_name> table is altered to have <required_partition_count> partitions
    Then the <table_name> table will have <required_partition_count> partitions
    And the <table_name> table will have the last_checked_timestamp column
