...
def build_query(args):
    query_connector_type = args["query-connector-type"].upper() if "query-connector-type" in args else "AND"
    ...
    for queryable_field in queryable_fields:
        if queryable_field[1] in args:
            ...
            queryable_options.append(f"{field_name} {comparison_operator} {value_to_check}")
...
if __name__ == "__main__":
    ...
    json_content = json.loads(open(os.path.join(abs_file_path, "event.json"), "r").read())
